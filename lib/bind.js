...
!function (){
  ...
  $b.render = function(templates, data, directive, bindCompleteHandler) {
    ...
    load(function(templates, data, directive) {
      ...
      bind.event = createEventManager();
      bind.functions = FunctionBinderList;

      bind.data = parseDataToBinderSet(data, bind);
  ...
  var createAction = function(selectorSpecifier){
  ...
  var loopNode = function(selectorSpecifier, Binder, directive, element) {
    ...
    if (exist(BoundDataMap[itemName])) {
      ...
      if (targetDirective.match('\\.')) {
        ...
        if ( rPath.length !== 0 ) {
          arrBinderSet = createArrayBinderSet(Binder);

          var binderSet;
          if (rPath.length > 1) {
            binderSet = createObjectBinderSet(rPath.slice(0, rPath.length), Binder);
      ...
      map(elements, function(element) {
        ...
        if (type === 'object') {
          ...
          action = function(value) {
            return createDomAction(selectorSpecifier, element)(
              targetDirective.call(this, value)
            );
          };
          Binder.functions.push( createFunctionBinder(action, Binder) );
        } else if (type === 'string') {
          action = createDomAction(selectorSpecifier, element);
          ...
          if (targetDirective.match('\\.')) {
            ...
            if (pPath.length != 0 && rPath.length != 0) {
              searchingResult.node.appendBinderSet(rPath[0], createObjectBinderSet(rPath, Binder));
              rootObjectBinderSet = BoundDataMap[pPath[0]];
            } else if (pPath.length == 0) {
              rootObjectBinderSet = createObjectBinderSet(path, Binder);
            ...
            if ( exist(BoundDataMap[targetDirective]) ) {
              ...
              binderSet = createBinderSet(undefined, Binder);
  ...
  var createFunctionBinder = function(f, Binder) {
    var b = createBinder(function(arg) {
      f.call(b, arg);
    });
    b.setEventHandler = function() {
      var handler = function(binder) {
        (b.get() !== f(binder)) && b.set(binder);
      };

      Binder.event.addEventListener('change', handler);
      Binder.event.addEventListener('append', handler);
      Binder.event.addEventListener('remove', handler);
    };
  ...
  var parseDataToBinderSet = function(data, Binder) {
    var parseArray = function(data) {
      var arrBinderSet = createArrayBinderSet(Binder);
      ...
      map(data, function(v, k) {
        ...
        if ( isArray(v) ) {
          ...
          arrBinderSet.appendBinderSet( createBinderSet(v, Binder) );
        }
      });
    ...
    var parseObject = function(data) {
      var objBinderSet = createObjectBinderSet(undefined, Binder);
      ...
      map(data, function(v, k) {
        ...
        if ( isArray(v) ) {
          ...
          objBinderSet.appendBinderSet( k, createBinderSet(v, Binder) );
        }
      });
    ...
    if ( isArray(data) ) {
      ...
      return createBinderSet(data, Binder);
    }
  };
  ...
  var createBinderSet = function(initialValue, Binder) {
    var v = initialValue;
    ...
    var bSet = function(value) {
      if ( exist(value) ) {
        ...
        Binder.event.dispatchEvent('change', Binder);
    ...
    bSet.clone = function() {
      var cloneBinderSet = createBinderSet(bSet(), Binder);
  ...
  var createObjectBinderSet = function(path, Binder) {
    var createOBSet = function() {
          ...
          if (typeof value === 'object') {
            map(value, function(v,k) {
              if ( 0 <= indexOf(keys,k) ) {
                ...
                bSet.appendBinderSet( k, parseDataToBinderSet(v, Binder) );
              }
            });
          }
          return value;
      ...
      bSet.clone = function() {
        var cloneBinderSet = createObjectBinderSet(undefined, Binder);
        map(keys, function(key) {
    ...
    if ( isArray(path) ) {
      if (1 < path.length) {
        ...
        bSet.appendBinderSet(path[1], createObjectBinderSet(path.slice(1, path.length), Binder));
        return bSet;
      } else {
        return createBinderSet(undefined, Binder);
      }
    } else {
      return createOBSet();
    }
  ...
  var createArrayBinderSet = function(Binder) {
    var bSets = [];
    ...
    bSet.clone = function() {
      var cloneBinderSet = createArrayBinderSet(Binder);
    ...
    bSet.remove = function(index) {
      if (0 <= index && index < bSets.length) {
        ...
        Binder.event.dispatchEvent('remove', Binder);
      }
    };
    ...
    bSet.push = function(arrayElementData) {
      var childBSet = bSet.templateBinderSetGeneratable()
            ? generateTemplateBinderSet()
            : parseDataToBinderSet(arrayElementData, Binder);
      bSet.appendBinderSet(childBSet);
      childBSet(arrayElementData);

      Binder.event.dispatchEvent('append', Binder);
