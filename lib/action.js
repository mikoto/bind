	...
	if (exist(BindedDataMap[loopDirective.specification.itemName])) {
		...
		if (targetDirective.match('\\.')) {
			...
			if ( rPath.length === 0 && arrBinderSet.hasTemplateBinderSet() ) {
				...
				parse(element.parentNode, BindedDataMap,
...
//						binderSet.appendBinderSet(rPath[rPath.length-1], arrBinderSet);
				...
				arrBinderSet.setTemplateBinderSet(
					BindedDataMap[loopDirective.specification.itemName]);
			}
...
//			element.parentNode.removeChild(element);
			...
			var binder = (function() {
				var createAction = (function() {
					var binderSet = undefined;
					
					var f = createDomAction(function(element, value) {
						parse(element, BindedDataMap,
							directive[selectorSpecifier.original][loopDirective.original]);
						
						var templateBinderSet = BindedDataMap[loopDirective.specification.itemName];
					...
					return function() {
						var fc = f.clone();
						fc.parentNode.appendChild(fc.element);
						return fc;
					};
				})();
				var action = createAction();
				action.parentNode.removeChild(action.element);
				return createBinder(action);
			})();
			...
			var templateBinderSet;
			if ( arrBinderSet.hasTemplateBinderSet() ) {
				templateBinderSet = arrBinderSet.getTemplateBinderSet();
			} else {
				if (loopDirective.elementType === 'Array') {
					templateBinderSet = createArrayBinderSet();
				} else if (loopDirective.elementType === 'Object') {
				...
				arrBinderSet.setTemplateBinderSet(templateBinderSet);
			}
			templateBinderSet.appendBinder(binder);
